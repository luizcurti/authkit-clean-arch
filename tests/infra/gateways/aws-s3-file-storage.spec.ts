...
describe('AwsS3FileStorage', () => {
  ...
  it('should config aws credentials and region on creation', () => {
  ...
  describe('upload', () => {
    ...
    it('should return imageUrl', async () => {
      ...
      expect(imageUrl).toBe(`https://${bucket}.s3.${region}.amazonaws.com/${fileName}`)
    })

    it('should return encoded imageUrl', async () => {
      const imageUrl = await sut.upload({ fileName: 'any file name', file })

      expect(imageUrl).toBe(`https://${bucket}.s3.${region}.amazonaws.com/any%20file%20name`)
    })
